...
-- Run from the repository root: lua5.4 tests/item_service_binding_spec.lua
...
test('getRequiredBindingKeys returns all registered keys', function()
    withFreshState(function(tables)
...
test('getRequiredBindingKeys returns empty table when no keys registered', function()
    withFreshState(function(tables)
        local keys = ItemService.getRequiredBindingKeys()
